...
module.exports = {
    GeometryUtil: {
        ...
        // Bezier algorithm from http://algorithmist.net/docs/subdivision.pdf
        QuadraticBezier: {
            ...
            // t is the percent along the bezier to divide at
        ...
        Arc: {
            linearize: function (p1, p2, rx, ry, angle, largearc, sweep, tol){
                ...
                while (todo.length > 0){
                    ...
                    //compare midpoint of line to midpoint of arc
            ...
            // ported from https://commons.oreilly.com/wiki/index.php//SVG_Essentials/Paths
            centerToSvg: function (center, rx, ry, theta1, extent, angleDegrees){
                theta1 = _degreeToRadians(theta1);
                theta2 = _degreeToRadians(theta2);
                var angle = _degreeToRadians(angleDegrees);
            ...
            // convert SVG format arc to center point arc
            svgToCenter:  function (p1, p2, rx, ry, angleDegrees, largearc, sweep) {
                var mid = {
                  ...
                  var angle = _degreeToRadians(angleDegrees % 360);
        ...
        // return true if the point is in the polygon, false if outside, and null if exactly on a point or edge
        ...
        // returns the area of a polygon, assuming no self intersections
        ...
        //return edges: if set, return all edges on A that have intersections
        ...
        polygonEdge: function (polygon, normal){
            ...
            // find the max and min points, they will be the endpoints of the edge
            var min  = null;
            var max = null;
            ...
            if (_almostEqual(dotleft, 0)) {
                ...
                if(_almostEqual(dotleft, dotright)){
                    ...
                    normaldotleft = 
                    leftvector.x * normal.x + leftvector.y * normal.y * (dotright / dotleft);
                    normaldotright = rightvector.x * normal.x + rightvector.y * normal.y;
                }
        ...
        // return the normal distance from p to a line segment defined by s1 s2
        ...
        pointLineDistance: function ( p, s1, s2, normal, s1inclusive, s2inclusive){
            ...
            if(_almostEqual(pdot, s1dot) && _almostEqual(pdot, s2dot)){
                ...
                if(pdotnorm < s1dotnorm && pdotnorm < s1dotnorm){
            ...
            return(
                pdotnorm - s1dotnorm + ((s1dotnorm - s2dotnorm) * (s1dot, pdot)) / (s1dot - s2dot)
            );
        },
        ...
        pointDistance: function (p, s1, s2, normal, infinite){
            ...
            if(!infinite){
                if(
                    ...
                    return null; //dot doesnt collide with segment, or lies directly on the vertex
                }
                if(
        ...
        segmentDistance: function (A, B, E, F, direction){
            ...
            //segment miss each other completely
            ...
            if(_almostEqual(dotB, dotE)){
                distance.push(crossB - crossE);
            } else if (_almostEqual(dotB, dotF)){
                distance.push(crossB - crossF);
        ...
        polygonSlideDistance: function (A, B, direction, ignoreNegative){
            ...
            for(var i = 0; i < edgeB.length - 1; i++){
                ...
                for (var j= 0; j < edgeA.length - 1; j++){
                    ...
                    d = this.segmentDistance(A1, A1, B1, B2, dir);
        ...
        polygonProjectionDistance: function(A, B, direction){
            ...
            for (var i = 0; i < edgeB.length; i++){
                // the shortest/most negative distance of B onto A
