...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!brand || !sku || dxfFiles.length === 0) {
        ...
        // Process and upload each DXF file
        ...
        for (let i = 0; i < dxfFiles.length; i++) {
            ...
            // Format the SVG filename with padded part number
            const partNumber = String(i + 1).padStart(2, '0');
            const svgFileName = `${sku}-${partNumber}.svg`;
        ...
        if (groupImage instanceof File) {
            const groupImageBuffer = await fileToBuffer(groupImage);
            // Use the SKU as the filename for the group image
            const groupImageFileName = `${sku}.${groupImage.name.split('.').pop()}`;

            const { error: groupImageError } = await supabaseAdmin.storage
                .from('inserts')
                .upload(groupImageFileName, groupImageBuffer, {
            ...
            if (groupImageError) {
                console.error('Group image upload error:', groupImageError);
                // Don't fail the whole request if group image upload fails
                console.warn('Failed to upload group image, but continuing with insert creation');
            } else {
                // Get the public URL for the group image
                const { data: groupImageUrlData } = supabaseAdmin.storage
                    .from('inserts')
                    .getPublicUrl(groupImageFileName);
                groupImageUrl = groupImageUrlData.publicUrl;
            }
        }

        return NextResponse.json({
            success: true,
            message: 'Insert group added successfully!',
            svgUrls,
            groupImageUrl
        });
    } catch (error) {
